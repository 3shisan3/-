...
{
	//判断有效性
	if(chessx>7||chessx<0||chessy>7||chessy<0)  //棋盘范围外
		return FALSE;
	if(board[chessx][chessy]==0)           //棋盘当前位置无棋子
		return judge(board,chessx,chessy,flag);  //能否下棋判断
	else 
		return FALSE;
}
...
{ 
	...
	//左上
	if(x>=2&&y>=2)
		if(tb[x-1][y-1]+flag==0)  //预计下棋位置左上方存在可翻转棋子（与预计将下棋子颜色不同）
		{
			x=x-2;
			y=y-2;
			while(x>=0&&y>=0)  
			{
				if(tb[x][y]==flag)   //存在和预计下棋子颜色相同的棋子 
					return TRUE;
				else if(tb[x][y]==-flag){x--;y--;}  //继续向左上寻找可达成翻转的棋子（颜色和将下棋子相同的棋子）
				else {x=-1;y=-1;}   //到达边界退出循环
			}
		}
	//上
	x=chessx;
...
int CBwfind::CountBlack()   //计算棋盘上黑棋数量，用于显示
{
	int CB=0;
	for(int i=0;i<8;i++)
		for(int j=0;j<8;j++)
...
int CBwfind::CountWhite()   //计算棋盘上白棋数量，用于显示
{
	int WB=0;
	for(int i=0;i<8;i++)
		for(int j=0;j<8;j++)
...
int CBwfind::CountNum()      //计算棋盘上总棋子数
{
	int Number=0,i,j;
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
...
int CBwfind::Total(int tb[][8])   //统计棋盘上剩余未落子数
{
	int Num=0,i,j;
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
...
int CBwfind::black(int tb[][8] )    //计数，用于估值
{
	int Bn=0,i,j;
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
...
int CBwfind::white(int tb[][8] )    //计数，用于估值
{
	int Wn=0,i,j;
		for(i=0;i<8;i++)
		for(j=0;j<8;j++)
...
BOOL CBwfind::search(int tb[][8] , int flag)  //确定是否存在某位置能够落子
{
	int i,j;
	for(i=0;i<8;i++)
		for(j=0;j<8;j++)
...
int CBwfind::findfirst(int tb[][8] , int &step, int &flag, int cut)  //估值函数，搜索最佳走法
{
...
int CBwfind::valuefirst(int tb[][8] )    //估值函数
{
	...
	//a
	DiffMobil=ComMobil-ManMobil;  //DiffMobil=ComMobil*ComMobil-ManMobil*ManMobil;
	//b
	Num=Total(tb);
	if(flag==1)   
	{
		minus=white(tb)-black(tb);  //白子数减黑子数
		if(minus>=0) 
			squre=minus*minus;
...
int CBwfind::findlast(int tb[][8], int &step, int &flag, int cut)    //搜寻下一步，manchoice人选择的棋子角色；comchoice电脑的棋子角色
{
	int value=0,max=-100000,min=100000,k=0,i,j;
	int temp[8][8];
	if(step==64){
		if(ComChoice==1)  //电脑为黑棋
			return(black(tb)-white(tb));
		else return(white(tb)-black(tb)); ///电脑白棋
	}
...
void CBwfind::newboard(int tb[][8],int chessx, int chessy, int flag) //棋盘棋子翻转
{
	int x=chessx,y=chessy;
	//左上
	tb[x][y]=flag;
...
void CBwfind::CornerSense(int tb[][8], int &value)   //估值函数
{	
	int flag,corner=0;
	//int i,j;
	flag=ComChoice;
...
int CBwfind::Mobility(int tb[][8], int flag)  //行动力
{
	int move=0,i,j;
	for(i=0;i<8;i++)for(j=0;j<8;j++){
...
void CBwfind::SaveBoard(int tb[][8])  //存储棋盘
{
	int i,j;
	bak++;
	for(i=0;i<8;i++)
...
void CBwfind::Corner(int tb[][8], int chessx, int chessy, int &value) //corner权值
{
	int flag=ComChoice;
	int corner=0;
